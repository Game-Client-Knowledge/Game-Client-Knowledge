...
## 1. 使用说明（以下为GPT生成标准回答，实际作答一般不用太详细，说出大概架构即可）
...
静态信息作为index独立服务，比如用户id，name，司机的id，name，支持的货运类型，状态等

##### 用户侧：

######  服务端：

######  数据层：req上下文存放动态信息，用户当前地理位置/下单填写位置，下单用户id
...
######  服务端：
 ...
 数据层：存放司机的实时位置信息（有个问题，分布式系统怎么存？总体来看还是拉个数据库存放好）
 服务层：实时位置上报服务，基础接单服务
...
1. 提供map服务，采用chunk划分地图，司机上报的位置信息以chunk划分做一个快速存储。
2. 提供快速查询附近司机服务，利用获取的用户req位置获取chunk，快速查询周围chunk的司机，获取一个小集合，随后采用优先队列，利用加权公式排序
